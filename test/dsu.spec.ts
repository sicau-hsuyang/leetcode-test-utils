...
describe("GeneralDsu", () => {
  ...
  test("Union and Find", () => {
    dsu.init([1, 2, 3, 4, 5]);
    dsu.union(1, 2);
    expect(dsu.find(1)).toBe(0); // 合并后应该返回 0
    expect(dsu.find(2)).toBe(0); // 合并后应该返回 0
    expect(dsu.find(3)).toBe(2);
    dsu.union(4, 5);
    expect(dsu.find(4)).toBe(3); // 合并后应该返回 3
    expect(dsu.find(5)).toBe(3); // 合并后应该返回 3
    dsu.union(1, 4);
    expect(dsu.find(1)).toBe(3); // 合并后应该返回 3
    expect(dsu.find(4)).toBe(3); // 合并后应该返回 3
  });
  ...
  test("Counter", () => {
    ...
    dsu.union(4, 5);
    dsu.union(1, 4);
    expect(dsu.counter()).toBe(1); // 所有元素合并后应该只剩下一个根节点
  });
